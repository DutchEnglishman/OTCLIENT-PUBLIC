...
-- Tibia-like: pressing Enter in walk (WASD) mode opens a temporary chat
-- (marked with '*') that returns to WASD after a single message is sent.
...
local transparentChatEnabled = false
local transparentChatColor = '#ffffff44'

-- Option: show underline/dots for highlighted text in console
local showHighlightedUnderline = false

local function applyConsoleTransparency()
    if not consolePanel then
        return
    end

    local color = transparentChatEnabled and transparentChatColor or 'white'
    if gameBottomPanel then
        gameBottomPanel:setImageColor(color)
    end

    consolePanel:setImageColor(color)

    local consoleBackground = consolePanel:getChildById('consoleBackground')
    if consoleBackground then
        consoleBackground:setImageColor(color)
    end

    if consoleContentPanel then
        consoleContentPanel:setImageColor(color)
    end

    if readOnlyPanel then
        readOnlyPanel:setImageColor(color)
        local panel = readOnlyPanel:getChildById('panel')
        if panel then
            panel:setImageColor(color)
        end
    end

    if consoleTabBar and consoleTabBar.tabs then
        for _, tab in pairs(consoleTabBar.tabs) do
            if tab.tabPanel then
                ...
                if consoleBuffer then
                    consoleBuffer:setImageColor(color)
                end
            end
        end
    end
...
function switchChat(enabled)
    ...
    if enabled then
        ...
        consoleToggleChat:setTooltip(
            tr('Disable chat mode, allow to walk using WASD')
        )
    ...
    if isChatEnabled() and consoleToggleChat.isChecked then
        ...
        if message == '' then
            if not isChatEnabled() then
                -- opening chat from walk mode: make it temporary ('*'),
                -- so a single Enter later sends the message and returns to WASD.
                walkAfterSend = true
                toggleChat()
            elseif walkAfterSend then
...
function load()
    ...
    if transparentButton then
        transparentButton:setChecked(transparentChatEnabled)
    end
    applyConsoleTransparency()
...
function applyTurnModifierSetting()
    if not g_game.isOnline() or not consoleToggleChat then
        return
    end
...
function addTab(name, focus)
    ...
    if tab.tabPanel then
        local consoleBuffer = tab.tabPanel:getChildById('consoleBuffer')
        if consoleBuffer then
            consoleBuffer:setImageColor(transparentChatEnabled and transparentChatColor or 'white')
        end
    end
...
function extendedViewTransparent(bool)
    transparentChatEnabled = bool
    applyConsoleTransparency()
end
