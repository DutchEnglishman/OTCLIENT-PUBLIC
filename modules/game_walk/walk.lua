...
local function getWasdTurnModifier()
    if modules and modules.client_options and modules.client_options.getOption and modules.client_options.getOption('turnCharacterWithShiftWASD') then
        return "Shift"
    end
...
local function getWasdTurnKey(key)
    return string.format("%s+%s", getWasdTurnModifier(), key)
end

local wasdTurnBindings = {}

local function isWasdTurnModifierPressed()
    local keyboardModifiers = g_keyboard.getModifiers()
    if getWasdTurnModifier() == "Shift" then
        return bit.band(keyboardModifiers, KeyboardShiftModifier) ~= 0
    end

    return bit.band(keyboardModifiers, KeyboardCtrlModifier) ~= 0
end
...
function bindWasdTurnKeys()
    ...
    for key, dir in pairs(keyToDirection) do
        local keyDownCallback = function()
            if isWasdTurnModifierPressed() then
                turn(dir, false)
            end
        end
        local keyPressCallback = function()
            if isWasdTurnModifierPressed() then
                turn(dir, true)
            end
        end
        local keyUpCallback = function()
            if isWasdTurnModifierPressed() then
                local player = g_game.getLocalPlayer()
                if player then
                    player:lockWalk(200)
        ...
        wasdTurnBindings[key] = {
            down = keyDownCallback,
            press = keyPressCallback,
            up = keyUpCallback
        }

        g_keyboard.bindKeyDown(key, keyDownCallback, gameRootPanel)
        g_keyboard.bindKeyPress(key, keyPressCallback, gameRootPanel)
        g_keyboard.bindKeyUp(key, keyUpCallback, gameRootPanel)
    end

    bindTurnKey(getWasdTurnKey("W"), North)
    bindTurnKey(getWasdTurnKey("D"), East)
    bindTurnKey(getWasdTurnKey("S"), South)
    bindTurnKey(getWasdTurnKey("A"), West)
end
...
function unbindWasdTurnKeys()
    local gameRootPanel = modules.game_interface.getRootPanel()
    for key, callbacks in pairs(wasdTurnBindings) do
        g_keyboard.unbindKeyDown(key, callbacks.down, gameRootPanel)
        g_keyboard.unbindKeyPress(key, callbacks.press, gameRootPanel)
        g_keyboard.unbindKeyUp(key, callbacks.up, gameRootPanel)
        wasdTurnBindings[key] = nil
    end

    for _, modifier in ipairs({ "Control", "Shift" }) do
        unbindTurnKey(modifier .. "+W")
        unbindTurnKey(modifier .. "+D")
        unbindTurnKey(modifier .. "+S")
        unbindTurnKey(modifier .. "+A")
    end
end
