...
local actionBarPanelTint = '#ffffff88'

function getMinimumChatHeight()
    local density = 1
    if g_window.getDisplayDensity then
        density = g_window.getDisplayDensity() or 1
    end
...
local function applyActionBarTransparency()
    ...
    gameBottomActionPanel:setImageColor(actionBarPanelTint)

    local lockLeftPanel = gameRootPanel and gameRootPanel:recursiveGetChildById('lockLeftPanel')
    local lockRightPanel = gameRootPanel and gameRootPanel:recursiveGetChildById('lockRightPanel')
    local lockPanel = gameRootPanel and gameRootPanel:recursiveGetChildById('lockPanel')
    if lockLeftPanel then
        lockLeftPanel:setImageColor(actionBarPanelTint)
    end
    if lockRightPanel then
        lockRightPanel:setImageColor(actionBarPanelTint)
    end
    if lockPanel then
        lockPanel:setImageColor(actionBarPanelTint)
    end
end

local function updateSidePanelButtons()
    local clientOptions = modules.client_options
    if not clientOptions or not clientOptions.getOption then
        leftIncreaseSidePanels:setEnabled(true)
        leftDecreaseSidePanels:setEnabled(not g_platform.isMobile())
        rightIncreaseSidePanels:setEnabled(true)
        rightDecreaseSidePanels:setEnabled(false)
        return
    end

    leftIncreaseSidePanels:setEnabled(not clientOptions.getOption('showLeftExtraPanel'))
    if g_platform.isMobile() then
        leftDecreaseSidePanels:setEnabled(false)
    else
        leftDecreaseSidePanels:setEnabled(
            clientOptions.getOption('showLeftPanel') or
            clientOptions.getOption('showLeftExtraPanel'))
    end
    rightIncreaseSidePanels:setEnabled(not clientOptions.getOption('showRightExtraPanel'))
    rightDecreaseSidePanels:setEnabled(clientOptions.getOption('showRightExtraPanel'))
end
...
function init()
    ...
    gameMainRightPanel = gameRootPanel:getChildById('gameMainRightPanel') or gameRightPanel
    ...
    gameBottomPanel:getChildById('bottomResizeBorder'):setMinimum(getMinimumChatHeight())
    ...
    Keybind.bind("Movement", "Stop All Actions", {
        {
            ...
            callback = function()
                if modules.game_hotkeys
                    and modules.game_hotkeys.isUiHotkeyFocusBlocked
                    and modules.game_hotkeys.isUiHotkeyFocusBlocked() then
                    return
                end
...
local function restoreTargetCursor()
    if not targetCursorActive then
        return
    end
...
local function setTargetCursor()
    -- Refresh the previous target state before starting a new action.
    restoreTargetCursor()
...
function onMouseGrabberRelease(self, mousePosition, mouseButton)
    ...
    if mouseButton == MouseLeftButton then
        local clickedWidget =
            gameRootPanel:recursiveGetChildByPos(
                mousePosition,
                false
            )
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature)
    ...
    -- Bare ground/border tiles always resolve to a non-nil "useThing" (the
    -- ground item itself), which made clicking on empty floor try to "use"
    -- the floor instead of walking to it. The server only accepts use
    -- requests within 1 tile and silently ignores them otherwise, so treat
    -- plain ground as "nothing to use" and let the click fall back to
    -- walking/autowalk instead.
    if useThing and not useThing:isContainer() and not useThing:isLyingCorpse() and
        not useThing:isMultiUse() and not useThing:isForceUse() and
        (useThing:isGround() or useThing:isGroundBorder()) then
        useThing = nil
    end
...
function getMainRightPanel()
    if not gameMainRightPanel then
        gameMainRightPanel = gameRightPanel
    end
