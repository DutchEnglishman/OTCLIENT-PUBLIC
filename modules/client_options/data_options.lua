return {
    ...
    turnCharacterWithShiftWASD        = {
        value = false,
        action = function(value, options, controller, panels, extraWidgets)
            -- setOption applies option.value after this action callback,
            -- so defer to use the updated value and avoid inverted behavior.
            scheduleEvent(function()
                if modules and modules.game_console and modules.game_console.applyTurnModifierSetting then
                    modules.game_console.applyTurnModifierSetting()
                end
            end)
        end
    },
    ...
    showSpellCooldowns                = {
        value = true,
        action = function(value, options, controller, panels, extraWidgets)
            modules.game_cooldown.setSpellCooldownsVisible(value)
        end
    },
    dontStretchShrink                 = {
        value = false,
