...
local function setup()
    ...
    -- Migration: restore normal mode when prior configs were set to heavy
    -- floor render modes that can make movement/animation feel too fast.
    if options.floorViewMode and (options.floorViewMode.value == 4 or options.floorViewMode.value == 1) then
        setOption('floorViewMode', 0, true)
    end

    -- Force shadow floor intensity to 100% regardless of saved preference.
    if options.shadowFloorIntensity and options.shadowFloorIntensity.value ~= 100 then
    ...
    if options.nativeCursor and not options.nativeCursor.value then
        setOption('nativeCursor', true, true)
    end
