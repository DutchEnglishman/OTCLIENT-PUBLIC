        ...
        if actionBar:isVisible() then
            ...
            for i, button in ipairs(children) do
                if i >= firstIndex and i < firstIndex + visibleCount then
                    -- Lazy instantiation check: if button is a placeholder (no item property), inflate it
                    if not button.item then
                         modules.game_actionbar.updateButton(button)
                         -- updateButton replaces the widget in the hierarchy but 'button' var is now stale/destroyed.
                         -- We need to get the new widget which replaced it at the same index.
                         -- Since tabBar layout order matches child order (usually), and updateButton used moveChildToIndex,
                         -- the new button should be at index 'i'.
                         
                         -- Refresh children list might be expensive, so just get by index if possible
                         local newButton = tabBar:getChildByIndex(i)
                         if newButton then
